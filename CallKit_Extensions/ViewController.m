...
-(void)checkPermissions:(UIButton *)sender
{
    ...
    // 获取权限状态
    [manager getEnabledStatusForExtensionWithIdentifier:@"com.ucpaasforiosdev.CallKit-Extensions.CallExtensions" completionHandler:^(CXCallDirectoryEnabledStatus enabledStatus, NSError * _Nullable error) {
        if (!error) {
            NSString *title = nil;
...
{
    CXCallDirectoryManager *manager = [CXCallDirectoryManager sharedInstance];
    [manager reloadExtensionWithIdentifier:@"com.ucpaasforiosdev.CallKit-Extensions.CallExtensions" completionHandler:^(NSError * _Nullable error) {
        if (error == nil) {
            UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"提示"
                                                                           message:@"更新成功"
                                                                    preferredStyle:UIAlertControllerStyleAlert];
